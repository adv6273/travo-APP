...
import { Navigate, useNavigate, useParams, useRouteError, useHistory } from "react-router-dom";
...
import axios from "axios";
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          setMData(data);
          //  {data.photos.length>0 && setPhotos(data.photos) } ;
        } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Failed to update place due to ', err,'. Check the console for more details.');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
        ...
        alert(response.data.message || 'Place deleted successfully showing frm frntend'); // Show backend response message in alert
        navigate('/acounts/myAccommodation'); // Redirect to places list after deletion
        // <Navigate to={'/acounts/myAccommodation'} />;
    } catch (err) {
        alert(err); // Show backend error message in alert
    }
};
      ...
      <div className="text-center mt-4">
                <div className="inline-flex space-x-4">
                    <button
                        ...
                        className="bg-primary py-2 px-4 text-white font-semibold rounded-full shadow-lg hover:bg-primary-dark transition duration-300"
                        type="submit"
                    >
                        Update
                    </button>
